...
add_action( 'admin_print_styles', 'suptic_admin_enqueue_styles' );

function suptic_admin_enqueue_styles() {
	global $plugin_page, $pagenow;

	if ( 'index.php' == $pagenow ) {
		wp_enqueue_style( 'suptic-dashboard', suptic_plugin_url( 'admin/dashboard-styles.css' ),
	...
	wp_enqueue_style( 'suptic', suptic_plugin_url( 'admin/styles.css' ),
		array(), SUPTIC_VERSION, 'all' );
}

add_action( 'admin_print_scripts', 'suptic_admin_enqueue_scripts' );

function suptic_admin_enqueue_scripts() {
	global $plugin_page;
...
function suptic_admin_init() {
	if ( isset( $_POST['suptic-create-form'] ) ) {
		...
		$form = suptic_insert_form( array(
			...
			'registered_only' => $_POST['registered-only'],
			'no_acceptance' => $_POST['no-acceptance'] ) );
