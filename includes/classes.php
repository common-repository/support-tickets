...
class SupTic_Form {
	...
	function update_meta( $meta_key, $new_value ) {
		...
		$wpdb->update( $table,
			...
			array( 'object_type' => 'form', 'object_id' => $this->id, 'meta_key' => $meta_key ),
			'%s', array( '%s', '%d', '%s' ) );
	...
	function get_meta( $meta_key, $single = true ) {
		...
		$table = suptic_db_table( 'meta' );
		$query = "SELECT meta_value FROM $table WHERE object_type LIKE 'form'"
...
function suptic_insert_form( $formarr = array() ) {
	...
	if ( $form->save() ) {
		$form->update_meta( 'registered_only', $formarr['registered_only'] );
		$form->update_meta( 'no_acceptance', $formarr['no_acceptance'] );
